...
class LinkData {
    ...
    constructor(fullName: string, description: string, url: string) {
...
const LINKS: {[name: string]: LinkData | undefined} = {
    "playground": new LinkData(
        "Layout Playground",
        "An interactive web-based analyzer",
        "https://o-x-e-y.github.io/layouts/playground"
    ),
    "keysolve": new LinkData(
        "KeySolve",
        "Another web-based analyzer",
        "https://clemenpine.github.io/keysolve-web/"
    ),
    "cyanophage": new LinkData(
        "cyanophage",
        "The layout playground and analyzer",
        "https://cyanophage.github.io/index.html"
    ),
    "klb": new LinkData(
        "Keyboard Layout Battles",
        "Analyzer which compares two layouts",
        "https://keyboard-layout-battles.fly.dev/"
    ),
    "layouts doc": new LinkData(
        "Keyboard Layouts Doc",
        "Many known layouts compiled in one place, sorted by SFB rate",
        "https://bit.ly/keyboard-layouts-doc"
    ),
    "big bag": new LinkData(
        "Dreymar's Big Bag",
        "A collection of modifications and other useful tricks",
        "https://dreymar.colemak.org/"
    ),
    "words filter": new LinkData(
        "gfruit's Words Filter",
        "A simple yet incredibly useful tool for finding words with certain bigrams, characters, and fingers",
        "https://gfruit.github.io/typing/words-filter.html"
    ),
    "keymap creator": new LinkData(
        "Octa's Keymap Creator",
        "A tool for creating and sharing layout fingermaps",
        "https://keymap-creator--octatypes.repl.co/"
    ),
    "fingermap": new LinkData(
        "Monkeytype Fingermap",
        "Another tool for creating and sharing layout fingermaps, made by the creator of MonkeyType",
        "https://fingermap.monkeytype.com/"
    ),
    "msklc guide": new LinkData(
        "MSKLC Guide",
        "Documentation for the Windows layout creator tool",
        "https://msklc-guide.github.io/#2"
    ),
    "guide to alt layouts": new LinkData(
        "Guide to alt layouts",
        "A helpful introduction to alt layouts",
        "https://getreuer.info/posts/keyboards/alt-layouts/"
    )
}

function getLinkData(name: string): LinkData {
    return LINKS[name] ?? new LinkData("", "", "");
}

function getLinkIcon(iconName: string): string {
    switch (iconName) {
